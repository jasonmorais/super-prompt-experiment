	...
	render: () => (
		...
		>
			<Routes>
				<Route
					path="/"
					element={<div data-testid="home">Public Content</div>}
				/>
				<Route
					path="/private"
					element={
		...
		const InstrumentedProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
			...
			return (
				<AuthContext.Provider value={auth as AuthContextProps}>
					{children}
					{called && <div data-testid="signinRedirect-called">signinRedirect called</div>}
		...
		const AutoSignInProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
			...
			return (
				<AuthContext.Provider value={auth as AuthContextProps}>
					...
					{signinCalled && <div data-testid="signin-called">signinRedirect called</div>}
				</AuthContext.Provider>
			);
		};
	...
	render: () => {
		const Provider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
			...
			return (
				<AuthContext.Provider value={auth as AuthContextProps}>
					{children}
					{signinCalled && <div data-testid="signin-called">signinRedirect called (should be manual only)</div>}
				</AuthContext.Provider>
			);
		};
...
export const WithError: Story = {
	render: () => {
		const Provider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
			...
			return <AuthContext.Provider value={auth as AuthContextProps}>{children}</AuthContext.Provider>;
		};
