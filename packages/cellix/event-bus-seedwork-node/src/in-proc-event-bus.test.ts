...
const feature = await loadFeature(path.resolve(__dirname, 'features/in-proc-event-bus.feature'));
...
type TestEventAHandler = AsyncHandlerMock<{ testA: string }>;
type TestEventBHandler = AsyncHandlerMock<{ testB: string }>;

test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	let handler: TestEventHandler;
	let handler1: TestEventHandler;
	let handler2: TestEventHandler;
	let handlerA: TestEventAHandler;
	let handlerB: TestEventBHandler;
	let error: unknown;
