...
class DummyAggregateRoot extends AggregateRoot<TestProps, typeof passport> {
...
const typeConverter = vi.mocked({
	...
	toAdapter: vi.fn(),
	toPersistence: vi.fn(),
} as TypeConverter<TestMongoType, TestProps, typeof passport, DummyAggregateRoot>);
...
class TestMongoRepository extends MongoRepositoryBase<TestMongoType, TestProps, typeof passport, DummyAggregateRoot> {}
...
test.for(mongoRepositoryFeature, ({ Background, Scenario, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		vi.clearAllMocks();
		repo = new TestMongoRepository(passport, model, typeConverter, eventBus, session);
	...
	Scenario('Creating a repository instance with static create', ({ Given, When, Then }) => {
		...
		When('the static create method is called', () => {
			repoInstance = TestMongoRepository.create(passport, model, typeConverter, eventBus, session, TestMongoRepository);
		});
	...
	Scenario('Saving a non-deleted aggregate', ({ When, Then, And }) => {
		When('the repository is saved with a non-deleted aggregate', async () => {
			...
			aggregate = new DummyAggregateRoot(props, passport);
			vi.spyOn(aggregate, 'getDomainEvents').mockReturnValue([domainEvent1, domainEvent2]);
