...
export interface AssessmentSubmitCommand {
	assessmentId: string;
	learningRecordId: string;
	courseId: string;
	activityKey: string;
	timeSpentMinutes: number;
	responses: Domain.Contexts.Learning.Assessment.AssessmentResponse[];
}
...
export const AssessmentAttempt = (dataSources: DataSources, learnerId: string) => ({
	...
	submit: async (command: AssessmentSubmitCommand) => {
		...
		const attemptCount = await dataSources.readonlyDataSource.Delivery.AssessmentAttempt.AssessmentAttemptReadRepo.count({
			organizationId: assessmentRef.organizationId,
			learnerId,
		...
		await dataSources.domainDataSource.Learning.Assessment.AssessmentUnitOfWork.withScopedTransaction(async (assessmentRepository) => {
			...
			await dataSources.domainDataSource.Delivery.AssessmentAttempt.AssessmentAttemptUnitOfWork.withScopedTransaction(async (attemptRepository) => {
				const attempt = await attemptRepository.getNewInstance({ assessment, learnerId, courseId: command.courseId, activityKey: command.activityKey, responses: command.responses, result: score, attemptNumber: attemptCount + 1 });
				result = await attemptRepository.save(attempt);
			});
			if (score.passed)
				await dataSources.domainDataSource.Delivery.LearningRecord.LearningRecordUnitOfWork.withScopedTransaction(async (recordRepository) => {
					const record = await recordRepository.get(command.learningRecordId);
					if (record.courseId !== command.courseId || record.learnerId !== learnerId) throw new Error('Learning record does not match this assessment attempt');
					record.recordActivity(command.activityKey, command.timeSpentMinutes, score.score);
