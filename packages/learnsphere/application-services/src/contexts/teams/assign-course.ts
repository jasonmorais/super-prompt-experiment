...
export const assignCourse = ({ dataSources, passport, identity }: TeamsServiceDependencies) => async (input: { teamId: string; courseId: string; dueAt?: Date }): Promise<TeamCourseAssignmentResult> => {
	...
	for (const member of team.members) {
		await assign(dataSources)({ organizationId: team.organizationId, learnerId: member.learnerId, learnerDisplayName: member.displayName, learnerEmail: member.email, teamName: team.name, courseId: input.courseId, source: 'MANAGER_ASSIGNED', assignedBy: actorName(identity), assignmentId, ...(input.dueAt ? { dueAt: input.dueAt } : {}) });
	}
