...
const roleForClaims = (roles: readonly Domain.Contexts.User.StaffRole.StaffRoleEntityReference[], claims: readonly string[]) =>
	roles.find((role) =>
		claims.some((claim) => {
			const value = normalizeClaim(claim);
			return value === normalizeClaim(role.roleName) || value === normalizeClaim(role.enterpriseAppRole);
		}),
	);
export const StaffUser = (dataSources: DataSources) => ({
	createIfNotExists: async (command: StaffUserCreateIfNotExistsCommand): Promise<Domain.Contexts.User.StaffUser.StaffUserEntityReference> => {
		...
		await dataSources.domainDataSource.User.StaffUser.StaffUserUnitOfWork.withTransaction(DomainRuntime.PassportFactory.forSystem(), async (repository) => {
			const user = await repository.getNewInstance(command.externalId, command.firstName, command.lastName, command.email);
			user.requestCreate(user.id);
			user.requestRoleAssignment(matchingRole, user.id);
			if (command.organizationId.trim()) user.grantOrganizationScope({ organizationId: command.organizationId, includeDescendants: matchingRole.enterpriseAppRole === 'Staff.Manager' });
			created = await repository.save(user);
		});
	...
	list: () => dataSources.readonlyDataSource.User.StaffUser.StaffUserReadRepo.getAll(),
	assignRole: async (input: { staffUserId: string; roleId: string; actorStaffUserId: string }) => {
		...
		await dataSources.domainDataSource.User.StaffUser.StaffUserUnitOfWork.withScopedTransaction(async (staffUserRepository) => {
			const user = await staffUserRepository.getById(input.staffUserId);
			let role: Domain.Contexts.User.StaffRole.StaffRoleEntityReference | undefined;
			await dataSources.domainDataSource.User.StaffRole.StaffRoleUnitOfWork.withScopedTransaction(async (roleRepository) => {
				role = await roleRepository.getById(input.roleId);
			});
			if (!role) throw new Error(`StaffRole with id ${input.roleId} not found`);
			user.requestRoleAssignment(role, input.actorStaffUserId);
			result = await staffUserRepository.save(user);
		});
