...
export interface LearnerUserCreateIfNotExistsCommand { externalId: string; lastName: string; restOfName?: string; email: string; }
...
export const LearnerUser = (dataSources: DataSources) => ({
	createIfNotExists: async (command: LearnerUserCreateIfNotExistsCommand): Promise<Domain.Contexts.User.LearnerUser.LearnerUserEntityReference> => {
		...
		await dataSources.domainDataSource.User.LearnerUser.LearnerUserUnitOfWork.withTransaction(DomainRuntime.PassportFactory.forSystem(), async (repository) => { created = await repository.save(await repository.getNewInstance(command.externalId, command.lastName, command.restOfName, command.email)); });
