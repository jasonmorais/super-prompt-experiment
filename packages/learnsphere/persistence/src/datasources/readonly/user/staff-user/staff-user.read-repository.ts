...
export interface StaffUserReadRepository {
	...
	getById(id: string): Promise<Domain.Contexts.User.StaffUser.StaffUserEntityReference | null>;
...
export const getStaffUserReadRepository = (models: ModelsContext, passport: Domain.Passport): StaffUserReadRepository => {
	...
	return {
		...
		getById: async (id) => {
			const doc = await models.StaffUser.findById(id).populate('role').exec();
			return doc ? converter.toDomain(doc, passport) : null;
		},
		getByExternalId: async (externalId) => {
			const doc = await models.StaffUser.findOne({ externalId }).populate('role').exec();
			return doc ? converter.toDomain(doc, passport) : null;
		},
		getByEmail: async (email) => {
			const doc = await models.StaffUser.findOne({ email }).populate('role').exec();
			return doc ? converter.toDomain(doc, passport) : null;
		},
	};
};
