...
export const getStaffRoleReadRepository = (models: ModelsContext, passport: Domain.Passport): StaffRoleReadRepository => {
	...
	return {
		getAll: async () => (await models.StaffRole.find({}).sort({ roleName: 1 }).exec()).map((doc) => converter.toDomain(doc, passport)),
		getById: async (id) => {
			const doc = await models.StaffRole.findById(id).exec();
