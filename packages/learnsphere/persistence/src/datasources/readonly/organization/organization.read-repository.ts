...
export const getOrganizationReadRepository = (models: ModelsContext, passport: Domain.Passport): OrganizationReadRepository => {
	...
	return {
		getByExternalId: async (externalId) => {
			const document = await models.Organization.findOne({ externalId }).exec();
		...
		listAccessible: async (organizationIds) =>
			(
				await models.Organization.find({ externalId: { $in: organizationIds } })
					.sort({ ancestorOrganizationIds: 1, name: 1 })
					.exec()
		...
		resolveAccessibleIds: async (scopes) => {
			...
			const documents = await models.Organization.find({ $or: [{ externalId: { $in: exact } }, { ancestorOrganizationIds: { $in: inheritedRoots } }] })
				.select({ externalId: 1 })
				.lean()
				.exec();
