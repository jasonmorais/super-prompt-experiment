...
export const getAssessmentReadRepository = (models: ModelsContext, passport: Domain.Passport): AssessmentReadRepository => {
	...
	const canRead = (assessment: Domain.Contexts.Learning.Assessment.AssessmentEntityReference): boolean => passport.learning.forAssessment(assessment).determineIf((permissions) => permissions.isSystemAccount || permissions.canManageAssessments || (permissions.canTakeAssessments && assessment.status === 'PUBLISHED'));
	return {
		getById: async (id) => {
		...
		list: async (organizationId, options) => {
			...
			const documents = await models.Assessment.find({ organizationId, ...(options?.status ? { status: options.status } : {}) }).sort({ updatedAt: -1 }).exec();
