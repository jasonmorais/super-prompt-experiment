...
{
	override async get(id: string) { const document = await this.model.findById(id).populate('modules.lessons.assessment').exec(); if (!document) throw new Error(`Course with id ${id} not found`); return this.typeConverter.toDomain(document, this.passport); }
	override async save(item: Domain.Contexts.Learning.Course.Course<CourseDomainAdapter>) { const saved = await super.save(item); return saved.isDeleted ? saved : this.get(saved.id); }
