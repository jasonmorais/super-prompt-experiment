...
export class AssessmentConverter extends MongooseSeedwork.MongoTypeConverter<AssessmentDocument, AssessmentDomainAdapter, Domain.Passport, Domain.Contexts.Learning.Assessment.Assessment<AssessmentDomainAdapter>> {
	constructor() {
		super(AssessmentDomainAdapter, Domain.Contexts.Learning.Assessment.Assessment);
	}
}
export class AssessmentDomainAdapter extends MongooseSeedwork.MongooseDomainAdapter<AssessmentDocument> implements Domain.Contexts.Learning.Assessment.AssessmentProps {
	...
	get passingScore() {
		return this.doc.passingScore;
	}
	set passingScore(value) {
		this.doc.passingScore = value;
	}
	get maxAttempts() {
		return this.doc.maxAttempts;
	}
	set maxAttempts(value) {
		this.doc.maxAttempts = value;
	}
	get questions() {
		return this.doc.questions.map((question) => ({
			key: question.key,
