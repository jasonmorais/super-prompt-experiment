...
export const TeamGoalContainer = () => {
	...
	const view = operation ? <TeamGoalDetail operation={operation} learnerId={identity.sub} submitting={submitting.loading} commenting={commenting.loading} attaching={attaching.loading} onSubmit={(candidate, note, evidence) => void submit(candidate, note, evidence)} onComment={(candidate, body) => void comment(candidate, body)} onAttach={(candidate, file) => void attach(candidate, file)} onBack={() => navigate('/operations')} /> : <Alert type="warning" showIcon message="Team goal not found" description="This team goal may have been completed, cancelled, or removed." action={<button type="button" onClick={() => navigate('/operations')}>Back to team operations</button>} />;
