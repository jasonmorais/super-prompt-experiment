...
const Protected = ({ children, required }: ProtectedProps) => {
	...
	return authorization.loading ? (
		<Spin size="large" />
	) : authorization.roleName && (!required || authorization.capabilities[required]) ? (
		children
	) : (
		<Navigate
			to="/unauthorized"
...
const StaffEntry = () => {
	...
	return (
		<Navigate
			to={getDefaultStaffPath(authorization.capabilities)}
			replace
		/>
	);
};
...
const Unauthorized = () => {
	const auth = useAuth();
	const authorization = useStaffAuthorization();
	if (auth.isLoading || authorization.loading)
		return (
			<div className="grid min-h-screen place-items-center">
				<Spin size="large" />
			</div>
	...
	if (authorization.roleName && recoveryPath !== '/unauthorized')
		return (
			<Navigate
				to={recoveryPath}
				replace
			...
			<Routes>
				...
				>
					...
					<Route
						...
						element={
							<Protected required="canManageAssessments">
								<AssessmentAuthoring />
							</Protected>
						}
					/>
